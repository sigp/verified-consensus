...
definition "lib_bool_sets_rel x = Sigma UNIV (\<lambda>v. lib_bool_sets x {v}) "
end

locale locals = locals_pre +
  assumes lib_rel_first : "F\<^sup>R\<^sub>x r = lib_bool_sets_rel x O r" 
  assumes lib_rel_last : "L\<^sup>R\<^sub>x r = r O lib_bool_sets_rel x"
begin

lemma last_univ: "L\<^sup>R\<^sub>x (id(x)) = UNIV"
  apply (clarsimp simp: lib_rel_last id_univ lib_bool_sets_rel_def)
